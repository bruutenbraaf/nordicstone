...
<?php if ( have_rows( 'content' ) ): ?>
	<?php while ( have_rows( 'content' ) ) : the_row(); ?>
		<?php if ( get_row_layout() == 'uitgelichte_categorie' ) : ?>
			...
			<?php $categorie_ids = get_sub_field( 'categorie' ); ?>
						<?php
							...
							<div class="col-md-3">
								<img style="max-width:100%" src="<?php the_post_thumbnail_url( $size ); ?>">
		...
		<?php elseif ( get_row_layout() == 'uitgelicht_categorie_afbeelding_met_tekst' ) : ?>
			<section id="uitgelicht-categorie" style="background-image:url(<?php $afbeelding = get_sub_field( 'afbeelding' ); ?><?php if ( $afbeelding ) { ?><?php echo $afbeelding['url']; ?>
	<?php } ?>);">
				<?php the_sub_field( 'Tekst' ); ?>
				<?php $link = get_sub_field( 'link' ); ?>
				<?php if ( $link ) { ?>
...
<div class="container">
	<div class="row">
						<?php $uitgelicht_categorie_ids = get_field( 'uitgelicht_categorie' ); ?>
						<?php
							$args = array(
							'posts_per_page'   => 4,
							'post_type' => 'product',
							'tax_query' => array(
								...
								'taxonomy' => 'product_cat',
								'field' => 'term_id',
								'terms' => $uitgelicht_categorie_ids
								)
							)
							);
							$query2 = new WP_Query( $args );
